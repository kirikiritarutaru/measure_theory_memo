...
# 単調収束定理
**[Prf]** つづき
実際、$f_j \leq f_{j+1}$ より $E_j \subset E_{j+1}$dearu.

各 $x\in X$ に対し $f(x) \geq \phi(x) > \alpha \phi(x)$ であり、 $\lim_{j\rightarrow\infty}f_j(x) = f(x)$ なので、$f_j(x) > \alpha \phi(x)$ なる番号 $j$ が存在する。したがって、$x\in E_j$ 。
