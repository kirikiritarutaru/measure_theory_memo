...
1. $f_1 \leq f_2 \leq \cdots \leq f_j \leq \cdots$
2. $f=\lim_{j\to \infty}f_j$ となる $f$ が存在する。
...
1. ほとんど至るところ $f=\lim_{j\to \infty}f_j$ となる $f$ が存在する。
2. ある $X$ 上の**非負可積分**関数 $\Phi$ が存在し、任意の自然数 $j$ に対してほとんど至るところ $|f_j| \leq \Phi$ 。
