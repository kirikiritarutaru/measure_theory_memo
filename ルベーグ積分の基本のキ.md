...
このとき、
$$f_n \rightarrow f\ \text{in}\ L^1,\ \|f_n-f\|_1\rightarrow 0\ (n\rightarrow\infty)$$
などと書く。
...
# 関数列の収束（一様収束）
**[Prf]** ディニの定理の証明
$\left\{B(x,\delta_x) \middle| x\in K \right\}$ はコンパクト集合 $K$ の開被覆なので、有限部分被覆が存在する。これを $B(x_n,\delta_{x_n})\ (n=1,2,\dots,N)$ とする。
$j_{x_1}, \dots,j_{x_N}\le J$ となるような $J$ を選ぶ。
任意の $y\in K$ をとるとき、 $y$ は少なくとも１つの球 $B(x_n,\delta_{x_n})$ に含まれる。
$g_j(x)$ は単調減少列なので、
$$0 \le g_J(y) \le g_{j_{x_0}}(y) < \varepsilon$$
となる。したがって、すべての $y\in K$ に対して $0 \le g_j(y) < \varepsilon\ (j \ge J)$ となるので、 $g_j$ は $0$ に一様収束する。

---
# 関数列の収束
Q. 測度空間上で 関数列 $\{f_n\}$ が $f$ に収束しているとき、一様収束している場所 $Y$ の広さ・大きさを測ることはできないか？
A. エゴロフの定理

**[Thm]** エゴロフの定理
$(X,\Sigma,\mu)$ を測度空間とする。 $E\in \Sigma$ を測度有限な可測集合とし、可測関数列 $\{f_n\}$ は $E$ 上で有限値関数 $f$ に**概収束**するとする。このとき、任意の $\varepsilon>0$ に対して次の条件をみたす集合 $E_0$ が存在する。
1. $E_0 \subset E,\ \mu(E-E_0) < \varepsilon$
...
\end{aligned}$$
$E_p(q)$ は $\Sigma$ に属している。

ここで、任意の $q$ に対して、 $p\rightarrow \infty$ のとき
$$\mu(E-E_p(q)) \rightarrow 0$$
である。
...
実際、 $E_1(q) \subset E_2(q) \subset \cdots$ であり、 $x\in E$ なら $f_j(x) \rightarrow f(x)\ (j\rightarrow\infty)$ であるから、 $\lim_{p\rightarrow\infty} E_p(q) =E$ である。
したがって、測度の基本的性質より $\lim_{p\rightarrow\infty}\mu(E-E_p(q))=0$ 。

以上より、 $\varepsilon>0$ が与えられた時、 $q$ に対して $p_q$ を次の条件をみたすように十分大きくとることができる。
$$\mu(E-E_{p_q}(q)) < \frac{\varepsilon}{2^q}$$
...
実際、
$$\mu(E-E_0)= \mu\left( \bigcap^\infty_{q=1} \left\{E-E_{p_q}(q)\right\} \right) \le \sum^\infty_{q=1} \mu\left(E-E_{p_q}(q)\right) < \sum^\infty_{q=1}\frac{\varepsilon}{2^q} = \varepsilon$$
...
## 関数列の収束（測度収束と概収束）
**[Thm]**
$(X,\Sigma,\mu)$ を測度空間とし、 $E$ を測度有限な可測集合とする。
...
**[Prf]**
$\varepsilon>0$ を与える。
エゴロフの定理より、 $E_0\subset E$ と $N$ が存在し、 $j>N$ となる $j$ をとると、
...
## 関数列の収束（測度収束と概収束）
**[Prf]**
$x\notin N$ ならば、 $x\in \bigcup^\infty_{l=1}\bigcap^\infty_{k=l}N^c_k$ となるので、ある $l$ が存在して $x\in \bigcap^\infty_{k=l}N^c_k$ となる。すなわち、 $k\ge l$ のとき $|f_{n_k}(x)-f(x)|\le \frac{1}{2^k}$ である。
