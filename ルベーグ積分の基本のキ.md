...
# 単関数列と可測関数
**[Thm]**
$f$ を $0 \le f(x) \le \infty$ を満たす可測関数とする。次の条件をみたす可測単関数列 $\{s_j\}_{j\in \mathbb{N}}$ が存在する。
1. すべての $x$ に対して、$0\le s_1(x) \le s_2(x) \le \dots \rightarrow f(x)$
2. 各 $s_j$ は有界かつ $m(\{x| s_j(x) \not ={0}\}) < \infty$
...
（天下り的だが、）$\alpha \in (0,1)$ と $0\le \phi \le f$ となる単関数 $\phi$ を任意にとる。
$$
