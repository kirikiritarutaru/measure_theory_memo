...
$f$ を**可測関数**という。
$$
\{x \in X | a < f(x) \leq \infty \} \in \mathcal{M}
$$
...
# 単調収束定理
**[Prf]**
2 の仮定より、（$\infty$ も許して） $\lim_{j\rightarrow \infty} f_j(x) = f(x)$ は常に存在する。 可測関数の上限、下限、上極限、下極限のスライドより、$f$ は可測関数。

1 の仮定より、$0\leq f_j \leq f$ なので
$$
...
$$
を示す。実際、上式が示されれば、$\lim_{j\rightarrow\infty} \int _X f_j d\mu$ が存在して、定理が示される。

$\alpha \in (0,1)$ と $0\leq \phi \leq f$ となる単関数 $\phi$ を任意にとる。
$$
E_j = \{ x\in X | f_j(x) > \alpha \phi(x)\} = \{ x\in X | f_j(x) -\alpha \phi(x) >0 \}
$$
とおく、$f-\alpha \phi$ は可測なので、$E_j\in \mathcal{M}$であり、
$$
...
$\nu$ は $\mathcal{M}$ 上の測度となる。$f_j \ge \chi_{E_j}f_j \ge \alpha \chi_{E_j}\phi$ なので
$$
\int_X f_j d\mu \ge \int_X \alpha \chi_{E_j}\phi d\mu = \alpha \int_{E_j}\phi d\mu = \alpha \nu(E_j)
$$
...
# 単調収束定理
**[Prf]** つづき
$\phi$ も $0 \le \phi \le f$ をみたす任意の単関数なので、左辺の上限をとると、
$$
